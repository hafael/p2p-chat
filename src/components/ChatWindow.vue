    ...
    <div class="flex flex-col h-full w-full max-w-4xl bg-gray-800 rounded-lg shadow-xl">
      <header class="bg-gray-900 p-4 rounded-t-lg">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-white">Chat with {{ contactName }}</h2>
          <div class="flex items-center space-x-2">
              ...
              <button @click="toggleVerificationDetails" title="Verify Identity" class="p-1 rounded-full hover:bg-gray-700">
        ...
        <div v-if="showVerification" class="mt-4 p-4 bg-gray-800 rounded-md border border-gray-700">
          <p class="text-sm text-gray-300 mb-3">To ensure security, compare these codes with your contact over a trusted channel (e.g., voice call).</p>
          <div class="space-y-1">
            <p class="text-xs text-gray-400">Your Fingerprint:</p>
            <p class="font-mono text-lg text-cyan-400 tracking-wider">{{ myFingerprint }}</p>
          </div>
          <div class="mt-3 space-y-1">
            <p class="text-xs text-gray-400">Fingerprint of {{ contactName }}:</p>
